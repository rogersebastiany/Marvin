...
def docs_dir(tmp_path):
    ...
    with patch("docs_backend.DOCS_DIR", docs):
        yield docs
...
class TestSafePath:
    def test_valid_filename(self, docs_dir):
        from docs_backend import _safe_path
        with patch("docs_backend.DOCS_DIR", docs_dir):
...
class TestSearchDocs:
    def test_empty_query(self, docs_dir):
        from docs_backend import search_docs
        result = search_docs("")
    ...
    def test_milvus_primary_path(self, docs_dir):
        ...
        with patch.dict("sys.modules", {"memory": MagicMock()}) as _:
            import memory
            memory.search_doc_chunks.return_value = [
    ...
    def test_keyword_fallback_when_milvus_down(self, docs_dir):
        ...
        with patch.dict("sys.modules", {"memory": MagicMock()}) as _:
            import memory
            memory.search_doc_chunks.side_effect = Exception("down")
...
class TestListDocs:
    def test_lists_markdown_files(self, docs_dir):
        from docs_backend import list_docs
        result = list_docs()
...
class TestGetDocSummary:
    def test_returns_first_section(self, docs_dir):
        from docs_backend import get_doc_summary
...
class TestReadDoc:
    def test_reads_full_file(self, docs_dir):
        from docs_backend import read_doc
