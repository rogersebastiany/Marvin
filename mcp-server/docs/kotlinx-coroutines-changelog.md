...
\* Introduced the `Flow.chunked(size: Int): Flow>` operator that groups emitted values into groups of the given size (#1290).
...
\* `kotlinx-coroutines-test`: set the default timeout of `runTest` to 60 seconds, added the ability to configure it on the JVM with the `kotlinx.coroutines.test.default\_timeout=10s` (#3800).
...
Changelog for previous versions may be found in [CHANGES\_UP\_TO\_1.7.md](CHANGES\_UP\_TO\_1.7.md)