...
Coroutines basics | Kotlin Documentation[{"id":"suspending-functions","level":0,"title":"Suspending functions","anchor":"#suspending-functions"},{"id":"add-the-kotlinx-coroutines-library-to-your-project","level":0,"title":"Add the kotlinx.coroutines library to your project","anchor":"#add-the-kotlinx-coroutines-library-to-your-project"},{"id":"create-your-first-coroutines","level":0,"title":"Create your first coroutines","anchor":"#create-your-first-coroutines"},{"id":"coroutine-scope-and-structured-concurrency","level":0,"title":"Coroutine scope and structured concurrency","anchor":"#coroutine-scope-and-structured-concurrency"},{"id":"create-a-coroutine-scope-with-the-coroutinescope-function","level":1,"title":"Create a coroutine scope with the coroutineScope() function","anchor":"#create-a-coroutine-scope-with-the-coroutinescope-function"},{"id":"extract-coroutine-builders-from-the-coroutine-scope","level":1,"title":"Extract coroutine builders from the coroutine scope","anchor":"#extract-coroutine-builders-from-the-coroutine-scope"},{"id":"coroutine-builder-functions","level":0,"title":"Coroutine builder functions","anchor":"#coroutine-builder-functions"},{"id":"coroutinescope-launch","level":1,"title":"CoroutineScope.launch()","anchor":"#coroutinescope-launch"},{"id":"coroutinescope-async","level":1,"title":"CoroutineScope.async()","anchor":"#coroutinescope-async"},{"id":"runblocking","level":1,"title":"runBlocking()","anchor":"#runblocking"},{"id":"coroutine-dispatchers","level":0,"title":"Coroutine dispatchers","anchor":"#coroutine-dispatchers"},{"id":"comparing-coroutines-and-jvm-threads","level":0,"title":"Comparing coroutines and JVM threads","anchor":"#comparing-coroutines-and-jvm-threads"},{"id":"what-s-next","level":0,"title":"What\u0027s next","anchor":"#what-s-next"}]{
"@context": "http://schema.org",
"@type": "WebPage",
"@id": "https://kotlinlang.org/docs/coroutines-basics.html#webpage",
"url": "https://kotlinlang.org/docs/coroutines-basics.html",
"name": "Coroutines basics | Kotlin",
"description": "",
...
The examples on this page use the explicit `this` expression with the coroutine builder functions `CoroutineScope.launch()` and `CoroutineScope.async()`. These coroutine builders are [extension functions](extensions.html) on `CoroutineScope`, and the `this` expression refers to the current `CoroutineScope` as the receiver.
...
2. Mark functions that can pause and resume with the `suspend` keyword:
   ...
   While you can mark the `main()` function as `suspend` in some projects, it may not be possible when integrating with existing code or using a framework. In that case, check the framework's documentation to see if it supports calling suspending functions. If not, use [`runBlocking()`](#runblocking) to call them by blocking the current thread.
...
4. Use [`withContext(Dispatchers.Default)`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines/with-context.html#) to define an entry point for multithreaded concurrent code that runs on a shared thread pool:
   ...
   The suspending `withContext()` function is typically used for [context switching](coroutine-context-and-dispatchers.html#jumping-between-threads), but in this example, it also defines a non-blocking entry point for concurrent code. It uses the [`Dispatchers.Default` dispatcher](#coroutine-dispatchers) to run code on a shared thread pool for multithreaded execution. By default, this pool uses up to as many threads as there are CPU cores available at runtime, with a minimum of two threads.
...
For more information on how lambdas with receivers work in Kotlin, see [Function literals with receiver](lambdas.html#function-literals-with-receiver).
...
The `CoroutineScope.launch()` function returns a [`Job`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines/-job/) handle. Use this handle to wait for the launched coroutine to complete. For more information, see [Cancellation and timeouts](cancellation-and-timeouts.html#cancel-coroutines).
...
[Coroutines guide](coroutines-guide.html)[Coroutines and channels − tutorial](coroutines-and-channels.html)