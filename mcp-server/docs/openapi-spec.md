...
//<![CDATA[
function PM\_Postback(param) { if (window.top.HideScreenLockWarningAndSync) { window.top.HideScreenLockWarningAndSync(1080); } if(window.CMSContentManager) { CMSContentManager.allowSubmit = true; }; \_\_doPostBack('m$am',param); }
function PM\_Callback(param, callback, ctx) { if (window.top.HideScreenLockWarningAndSync) { window.top.HideScreenLockWarningAndSync(1080); }if (window.CMSContentManager) { CMSContentManager.storeContentChangedStatus(); };WebForm\_DoCallback('m$am',param,callback,ctx,null,true); }
//]]>
...
* [Products](#)
  ...
  across OpenAPI and AsyncAPI.](/product/)
  + [Studio
    Collaborate on API Design](/product/studio/)
  + [Portal
    Deliver Up-to-date API Documentation](/product/portal/)
  + [Explore
    Quickly Test and Explore APIs](/product/explore/)
  + [Functional Testing
    Automated API Testing](/product/functional-testing/)
  + [Contract Testing
    Block API Breaking Changes](/product/contract-testing/)[Swagger Enterprise

  Standardize your APIs with projects,
  style checks, and reusable domains.](/product/enterprise/)
* [Community](#)

  [Swagger Open Source

  Ideal for individuals and small teams to design, build, and document APIs.](/tools/open-source/)
  + [Codegen
    Generate server stubs and client SDKs from OpenAPI Specification definitions.](/tools/swagger-codegen/)
  + [Editor
    API editor for designing APIs with the OpenAPI and AsyncAPI specifications.](/tools/swagger-editor/)
  + [UI
    Visualize OpenAPI Specification definitions in an interactive UI.](/tools/swagger-ui/)
* [Learn](#)

  [API Design](/solutions/api-design/ "API Design")
  [API Development](/solutions/api-development "API Development")
  [API Documentation](/solutions/api-documentation/ "API Documentation")
  [API Testing](/solutions/api-testing/ "API Testing")
  [API Mocking and
  Virtualization](/solutions/mocking-and-virtualization/ "API Mocking and Virtualization")
  [API Governance](/solutions/governance-and-standardization "API Governance")
  [API Monitoring](/solutions/api-monitoring/ "API Monitoring")
  [OpenAPI & Swagger](/solutions/getting-started-with-oas/ "OpenAPI & Swagger")
* [Resources](#)

  [Resources](/resources/)

  [OpenAPI Specification](/resources/open-api/ "OpenAPI Specification")
  [Docs](/docs/ "Docs")
  [Blog](/blog/ "Blog")
...
* header .nav-item-search a::after {
  ...
  function initSearch() {
  var config = {
  locations: ['Swagger\_Website', 'Swagger\_Documentation', 'Community', 'Swagger\_Blog'],
...
* [Get started](/tools/swaggerhub/why-swaggerhub)
...
.link-item\_\_title,
.list-item\_\_description,
.content {
line-height: 20px;
}
.list-item\_\_title::before {
content: "";
pointer-events: none;
width: 10px;
height: 100px;
...
.list-item:first-of-type .list-item\_\_title::after {
content: "";
pointer-events: none;
width: 10px;
height: 34px;
...
.list-item:last-of-type .list-item\_\_title::before {
content: "";
pointer-events: none;
width: 10px;
height: 48px;
...
This specification also *uses* schemas in the form of the [Schema Object](#schema-object).
...
Behavior described as *undefined* is likely, at least in some circumstances, to result in outcomes that contradict the specification.
...
Behavior described as *implementation-defined* allows implementations to choose which of several different-but-compliant approaches to a requirement to implement.
...
Unlike undefined behavior, it is safe to rely on implementation-defined behavior if *and only if* it can be guaranteed that all relevant tools support the same behavior.
...
While some implementations support this sort of parsing due to the requirements of past versions of this specification, in version 3.1, the result of parsing fragments in isolation is *undefined* and likely to contradict the requirements of this specification.
...
A special case of parsing fragments of OAS content would be if such fragments are embedded in another format, referred to as an *embedding format* with respect to the OAS.
...
These connections are unambiguously resolved in single-document OADs, but the resolution process in multi-document OADs is *implementation-defined*, within the constraints described in this section.
...
URIs used as references within an OpenAPI Description, or to external documentation or other supplementary information such as a license, are resolved as *identifiers*, and described by this specification as ***URIs***.
...
For standalone JSON Schema documents that do not set `$schema`, or for Schema Objects in OpenAPI description documents that are *not* [complete documents](#openapi-description-structure), the dialect SHOULD be assumed to be the OAS dialect.
...
The `allOf` form of `discriminator` is *only* useful for non-validation use cases; validation with the parent schema with this form of `discriminator` *does not* perform a search for child schemas or use them in validation in any way.
...
The value of the property named in `propertyName` is used as the name of the associated schema under the [Components Object](#components-object), *unless* a `mapping` is present for that value.
...
When using arrays, XML element names are *not* inferred (for singular/plural forms) and the `name` field SHOULD be used to add that information.
...
document.addEventListener("DOMContentLoaded",function(){setTimeout(function(){const e=document.querySelector('a[href="https://www.apache.org/licenses/-l-i-c-e-n-s-e-2.0.html"]');e&&(e.href="https://www.apache.org/licenses/LICENSE-2.0.html")},2000)});

var elems = document.querySelectorAll('pre');
elems.forEach(function(e) {
e.className += "prettyprint linenums"
});
tocbot.init({
tocSelector: '.toc-list-wrapper',
contentSelector: '.col-md-9',
headingSelector: 'h2, h3, h4',
scrollSmooth: true,
scrollSmoothDuration: 420,
headingsOffset: 1,
ignoreSelector: '#fka-swagger-restful-api-documentation-specification',
orderedList: false
//collapsibleClass: 'footer\_brand\_logo',
});
$(document).ready(function () {
var urls = document.querySelectorAll('.specification\_page .col-md-9 a');
var i;
for (i = 0; i < urls.length; i++) {
url = urls[i].href;
urlCamel = url.replace(new RegExp('https://github.com/OAI/OpenAPI-Specification/blob/master/', 'g'), '');
urlCamelSplit = urlCamel.split(/(?=[A-Z])/);
urlOutput = "";
for(u=0; u < urlCamelSplit.length; u++){
if(u < urlCamelSplit.length-1){
urlOutput += urlCamelSplit[u].toLowerCase() + "-";
} else {
urlOutput += urlCamelSplit[u].toLowerCase();
}
urls[i].href = urlOutput
}
}
});
...
* [Partners](https://smartbear.com/partners/overview/)
...
* [Swagger](/product/)
* [Swagger
  Editor](/tools/swagger-editor/)
* [Swagger
  Codegen](/tools/swagger-codegen/)
* [Swagger UI](/tools/swagger-ui/)
...
* [Terms of
  Use](https://smartbear.com/terms-of-use/)
* [Website
  Terms of  Use](https://smartbear.com/website-terms-of-use/)
...
© 2026 SmartBear Software. All Rights
Reserved.