...
creating and starting threads using [`Thread`](#threading.Thread "threading.Thread"):

```
importthreading
importtime

defcrawl(link, delay=3):
...
[`concurrent.futures.ThreadPoolExecutor`](concurrent.futures.html#concurrent.futures.ThreadPoolExecutor "concurrent.futures.ThreadPoolExecutor") offers a higher level interface
...
[`queue`](queue.html#module-queue "queue: A synchronized queue class.") provides a thread-safe interface for exchanging data between
running threads.

[`asyncio`](asyncio.html#module-asyncio "asyncio: Asynchronous I/O.") offers an alternative approach to achieving task level
...
**CPython implementation detail:** In CPython, due to the [Global Interpreter Lock](../glossary.html#term-global-interpreter-lock), only one thread
can execute Python code at once (even though certain performance-oriented
libraries might overcome this limitation).
If you want your application to make better use of the computational
resources of multi-core machines, you are advised to use
[`multiprocessing`](multiprocessing.html#module-multiprocessing "multiprocessing: Process-based parallelism.") or [`concurrent.futures.ProcessPoolExecutor`](concurrent.futures.html#concurrent.futures.ProcessPoolExecutor "concurrent.futures.ProcessPoolExecutor").
However, threading is still an appropriate model if you want to run
multiple I/O-bound tasks simultaneously.

## GIL and performance considerations

Unlike the [`multiprocessing`](multiprocessing.html#module-multiprocessing "multiprocessing: Process-based parallelism.") module, which uses separate processes to
bypass the [global interpreter lock](../glossary.html#term-global-interpreter-lock) (GIL), the threading module operates
...
As of Python 3.13, [free-threaded](../glossary.html#term-free-threading) builds
...
:   Handle uncaught exception raised by [`Thread.run()`](#threading.Thread.run "threading.Thread.run").
    ...
    * *thread*: Thread which raised the exception, can be `None`.
    ...
    [`sys.excepthook()`](sys.html#sys.excepthook "sys.excepthook") handles uncaught exceptions.
...
threading.setprofile\_all\_threads(*func*)
:   Set a profile function for all threads started from the `threading` module
    and all Python threads that are currently executing.
...
You can create custom [`local`](#threading.local "threading.local") objects by subclassing the
`local` class:
...
`threading.excepthook()` ignores silently [`SystemExit`](exceptions.html#SystemExit "SystemExit").
...
:   This constructor should always be called with keyword arguments. Arguments
    ...
    *context* is the [`Context`](contextvars.html#contextvars.Context "contextvars.Context") value to use when starting
    the thread. The default value is `None` which indicates that the
    [`sys.flags.thread_inherit_context`](sys.html#sys.flags.thread_inherit_context "sys.flags.thread_inherit_context") flag controls the behaviour. If
    the flag is true, threads will start with a copy of the context of the
    caller of [`start()`](#threading.Thread.start "threading.Thread.start"). If false, they will start with an empty
    context. To explicitly start with an empty context, pass a new instance of
    [`Context()`](contextvars.html#contextvars.Context "contextvars.Context"). To explicitly start with a copy of the
    current context, pass the value from [`copy_context()`](contextvars.html#contextvars.copy_context "contextvars.copy_context"). The
    ...
    :   Start the thread’s activity.
        ...
        [`threading.Thread.name`](#threading.Thread.name "threading.Thread.name"). The name can be truncated depending on the
    ...
    :   Wait until the thread terminates. This blocks the calling thread until
        ...
        you must call [`is_alive()`](#threading.Thread.is_alive "threading.Thread.is_alive") after `join()` to
        ...
        `join()` raises a [`RuntimeError`](exceptions.html#RuntimeError "RuntimeError") if an attempt is made
        ...
        of [Python finalization](../glossary.html#term-interpreter-shutdown) `join()`
        raises a [`PythonFinalizationError`](exceptions.html#PythonFinalizationError "PythonFinalizationError").

        Changed in version 3.14: May raise [`PythonFinalizationError`](exceptions.html#PythonFinalizationError "PythonFinalizationError").
...
[`release()`](#threading.Lock.release "threading.Lock.release"). When the state is unlocked, `acquire()`
...
so it is recommended to use [`with`](../reference/compound_stmts.html#with) instead of manually calling
[`acquire()`](#threading.RLock.acquire "threading.RLock.acquire") and [`release()`](#threading.RLock.release "threading.RLock.release")
...
`release()` (the `release()` of the outermost pair) resets
...
:   This class implements reentrant lock objects. A reentrant lock must be
    ...
    :   Acquire a lock, blocking or non-blocking.
        ...
        :   Recommended over manual `acquire()` and [`release()`](#threading.RLock.release "threading.RLock.release") calls
            whenever practical.
        ...
        >   `RLock`; `Lock` handles this case the same as the previous,
...
[`notify_all()`](#threading.Condition.notify_all "threading.Condition.notify_all"). Once awakened, `wait()`
...
Note: the [`notify()`](#threading.Condition.notify "threading.Condition.notify") and [`notify_all()`](#threading.Condition.notify_all "threading.Condition.notify_all") methods
...
To choose between [`notify()`](#threading.Condition.notify "threading.Condition.notify") and [`notify_all()`](#threading.Condition.notify_all "threading.Condition.notify_all"),
...
:   This class implements semaphore objects. A semaphore manages an atomic
    ...
    [`acquire()`](#threading.Semaphore.acquire "threading.Semaphore.acquire") calls, plus an initial value. The `acquire()` method
...
### [`Semaphore`](#threading.Semaphore "threading.Semaphore") example
...
`release` methods can be used as context managers for a [`with`](../reference/compound_stmts.html#with)
...
Currently, [`Lock`](#threading.Lock "threading.Lock"), [`RLock`](#threading.RLock "threading.RLock"), [`Condition`](#threading.Condition "threading.Condition"),
[`Semaphore`](#threading.Semaphore "threading.Semaphore"), and [`BoundedSemaphore`](#threading.BoundedSemaphore "threading.BoundedSemaphore") objects may be used as
[`with`](../reference/compound_stmts.html#with) statement context managers.

---

## Bibliography

1. [`threading` — Thread-based parallelism](https://docs.python.org/3/library/threading.html)