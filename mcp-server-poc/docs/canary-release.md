...
# [Canary Release](CanaryRelease.html)

25 June 2014

[Danilo Sato](http://www.dtsato.com/)
...
Similar to a [BlueGreenDeployment](/bliki/BlueGreenDeployment.html), you start by deploying the
...
infrastructure using [PhoenixServers](/bliki/PhoenixServer.html) or to provision new
infrastructure and decommission the old one using [ImmutableServers](/bliki/ImmutableServer.html).

Canary release is an application of [ParallelChange](/bliki/ParallelChange.html), where the
...
the [FeatureFlags](/bliki/FeatureFlag.html) turned on so they can detect problems with new
features early.
...
use [ParallelChange](/bliki/ParallelChange.html) to support both versions and monitor which
...
Again, using [ParallelChange](/bliki/ParallelChange.html) is a technique to mitigate this
