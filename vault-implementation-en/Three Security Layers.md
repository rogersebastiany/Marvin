...
**1. Transport**
The channel through which data travels. In the POC, it is local [[stdio]] (secure by design -- pipe between processes on the same OS). In [[Production Architecture|production]]:
...
The POC has none of the 3 layers -- this is intentional by design. The goal is to demonstrate the [[Server Chain]] and the [[Deterministic Feedback Loop]], not security. The `verify=False` in [[web-to-docs]] is symptomatic: in a POC, simplifying is more valuable than securing.

## In the Thesis

"The gate is open, but the terrain around it is fenced."

[[MCP]] is O(1) by design -- instant access to [[Context]]. The 3 layers ensure that this instant access only happens for those who should have it. The [[Ontology as Code|ontology]] is protected without sacrificing performance.

In regulated domains (finance, healthcare, law), these 3 layers are not optional -- they are compliance requirements. The thesis's [[Measurable Determinism|determinism]] is only useful if the system is auditable and secure.

---

Related to: [[MCP]], [[MCP Gateway]], [[Production Architecture]], [[stdio]], [[web-to-docs]], [[Ontology as Code]], [[Tenant Isolation]], [[S3 as Persistent Ontology]]
