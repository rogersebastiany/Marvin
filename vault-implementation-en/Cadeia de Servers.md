...
The 4 MCP servers in the POC complement each other in a chain of cumulative [[Contexto Programático|context]]. Each server addresses a gap the previous one does not cover, and the output of one feeds the next.
...
1. [[Agente na POC]] needs information -> calls [[docs-server]] `search_docs("lambda")`
...
6. prompt-engineer injects the complete [[Catálogo de Tools]] into the generated prompt
...
**prompt-engineer -> all 3:** Imports the 3 sibling servers at startup to build the [[Catálogo de Tools]]. Tight coupling -- all 3 must be importable.

**system-design -> docs-server:** Reads `docs/mermaid-*.md` at startup as syntax reference. Depends on the docs existing.

## In the Thesis

The chain implements the progressive reduction of the [[Espaço Amostral]]:
...
Each link is an intersection operation in [[Teoria dos Conjuntos]]: A intersection T1 intersection T2 intersection T3 -> precise [[Subconjunto]] -> [[Determinismo Mensurável|determinism]].

It is [[Redução de Espaço na Prática]] materialized in software architecture.
...
The two new servers enable the [[Loop de Auto-Melhoria]]: the agent queries -> acts -> logs -> discovers -> enriches -> next cycle is richer.

---

Related to: [[docs-server]], [[web-to-docs]], [[prompt-engineer]], [[system-design]], [[mcp-ontology-server]], [[mcp-memory-server]], [[Feedback Loop Determinístico]], [[Redução de Espaço na Prática]], [[Catálogo de Tools]], [[Agente na POC]], [[Contexto Programático]], [[Loop de Auto-Melhoria]]
