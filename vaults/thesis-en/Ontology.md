...
Examples:
- Defining what a node is, what an edge is, and what a [[Complete Directed Graph]] is -- that is graph ontology
...
In the [[Ontological Tautology]] thesis, ontology is synonymous with complete [[Context]]. When all entities, relations, behaviors, and constraints of a domain are defined and accessible to the model via [[Tool|tools]] and [[MCP]], the model has the complete ontology of that domain.

The direct consequence: if the ontology is complete, [[Inference]] transforms into [[Deduction]], and the result is [[Tautology|tautological]].

## Ontology and the Sample Space

In [[Set Theory]], ontology defines which [[Subset]] of the [[Sample Space]] is relevant. Without ontology, the model operates in the entire space S -- high uncertainty, high risk of [[Hallucination]]. With complete ontology, it operates in a [[Subset]] A ⊂ S so precise that the correct answer is deducible.
...
A domain's ontology is complete when **every method/process of the domain has a corresponding [[Tautological Tool]]**. There is no method in the company or area that the agent would need to execute but has no tool for.

This is verifiable:
1. Enumerate all methods/processes of the domain
2. For each one, verify: does a corresponding tool exist?
3. For each tool, verify: is it [[Tautological Tool|tautological]]? (complete I/O contract, finite output, explicit failure)

If yes for all -> complete ontology -> [[Tautology]] by construction -> [[Determinism]] with accuracy.

Completeness is not abstract -- it is a concrete checklist.

---

Related to: [[Tautology]], [[Context]], [[Sample Space]], [[Set Theory]], [[Tool]], [[Tautological Tool]], [[MCP]], [[Determinism]], [[Hallucination]], [[Ontological Tautology]]
