...
The state in which, given an input and a [[Contexto]], the output is predictable and reproducible. There is no variation, no surprise -- the result is a necessary consequence of the initial conditions.
...
In computational systems, determinism means that the same input produces the same output, always. A pure function is deterministic. A hash is deterministic. An LLM with temperature zero and complete [[Contexto]] approaches deterministic behavior.
...
The paper [[LLM Output Drift]] formalizes this phenomenon: without rigid [[Contexto]], the AI varies responses even at temperature zero, especially cross-provider.
...
The paper [[Ultra-Long-Horizon Agentic Science]] shows that this number grows with [[Acumulação Cognitiva]] -- not linear aggregation, but progressive distillation of experience into knowledge into wisdom (L1->L2->L3).

The path: [[Ontologia]] -> [[Tautologia]] -> Determinism.
...
In the general case: determinism without complete [[Ontologia]] is merely consistency in error.
...
[[Convergência]] is the process of approaching determinism. The [[Loss Function]] converges during training -- the error decreases to a minimum. Analogously, [[Contexto]] converges the [[Espaço Amostral]] -- it reduces candidates until (ideally) one remains.

Determinism is the final state. [[Convergência]] is the path to it.
