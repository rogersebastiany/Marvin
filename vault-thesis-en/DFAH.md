...
Determinism-Faithfulness Assurance Harness. Paper by Raffi Khatchadourian (City University of New York) that measures [[Determinism]] and faithfulness of tool-using LLM agents, revealing that determinism and accuracy are independent dimensions.
...
The paper introduces a "replayable" harness -- it records agent trajectories (sequences of tool calls) and re-executes them to measure divergence. With structured [[Context]] (typed schemas, specs, defined tools), trajectory determinism rises to 89-90%+.

It is the central empirical proof of the [[Ontological Tautology]] thesis: [[Ontology]] (structured context) -> [[Tautology]] (predictable result) -> [[Determinism]] (89%+).
...
**However**: this null correlation applies to agents with generic tools -- tools whose output is ambiguous or open-ended. When tools are [[Tautological Tool|tautological]] (complete I/O contract, finite output, explicit failure), determinism **implies** accuracy. If the tool can only return the correct answer or "I don't know," a deterministic system is deterministically correct.
...
What produces the 89%+ is specifically **schema-first architecture**: typed tool definitions, parameters with explicit types, formatted returns. It is not generic "context." It is structured [[Context]] with contracts.
...
The 89% is not unconditional -- it requires maintaining schema and [[Context]] quality.
...
The paper [[Ultra-Long-Horizon Agentic Science]] shows that the number can grow with [[Cognitive Accumulation]] -- sustaining the "+".

---

Related to: [[Determinism]], [[Ontology]], [[Tautology]], [[LLM Output Drift]], [[Ultra-Long-Horizon Agentic Science]], [[Context]], [[Ontological Tautology]], [[Cognitive Accumulation]], [[FastMCP]], [[Tautological Tool]]
